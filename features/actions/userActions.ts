...
export const createUserActions = (deps: {
            ...
            if (nextLevelInfo.branchRequirements && nextLevelInfo.branchRequirements.length > 0) {
                for (const req of nextLevelInfo.branchRequirements) {
                    if (nextLevelInfo.level === 2) {
                        // Đặc biệt cho Cấp 2: Kiểm tra số lượng F1 Active
                        const activeF1Count = (user.children || []).filter(c => c.status === UserStatus.Active).length;
                        if (activeF1Count < req.count) {
                            branchReqsMet = false;
                            break;
                        }
                    } else {
                        // Cho các cấp khác: Kiểm tra số lượng nhánh F1 có người đạt rank yêu cầu
                        let qualifiedBranches = 0;
                        const flattenUsers = (list: AdminManagedUser[]): AdminManagedUser[] => list.flatMap(u => [u, ...(u.children ? flattenUsers(u.children) : [])]);
                        
                        (user.children || []).forEach(f1 => {
                            const branchUsers = [f1, ...flattenUsers(f1.children || [])];
                            const hasQualifiedMember = branchUsers.some(u => u.rankLevel >= req.targetLevel);
                            if (hasQualifiedMember) qualifiedBranches++;
                        });

                        if (qualifiedBranches < req.count) {
                            branchReqsMet = false;
                            break;
                        }
                    }
    ...
    const handleFullAddUser = (newUser: AdminManagedUser, parentId?: string): { success: boolean, message?: string } => {
        ...
        if (flatUsers.some(u => u.email.toLowerCase() === newUser.email.toLowerCase() || u.phone === newUser.phone)) {
    ...
    const handleUpgradeTier = (newTier: MembershipTier): UpgradeResult => {
        ...
        const fees = { [MembershipTier.Starter]: settingsState.systemSettings.participationFee, [MembershipTier.Pro]: settingsState.systemSettings.proParticipationFee, [MembershipTier.Master]: settingsState.systemSettings.masterParticipationFee, [MembershipTier.None]: 0 };
        const upgradeFee = fees[newTier];

        if (freshUser.balance < upgradeFee) return { success: false, needed: upgradeFee - freshUser.balance };

        const changes = calculateFeePaymentChanges(freshUser.id, 'participation', newTier, userState.allUsers, settingsState.systemSettings, settingsState.fundSettings);
        if(!changes) return { success: false };
        
        const bulkUpdates: any[] = [{
            id: freshUser.id,
            balance: freshUser.balance - upgradeFee,
            creditBalance: freshUser.creditBalance + (upgradeFee / 1000),
        ...
        Object.keys(changes.userUpdates).forEach(uid => {
            ...
            if (u) {
                const newEarnings = u.totalEarnings + changes.userUpdates[uid].earningsChange;
                const levelUp = checkAndApplyLevelUp(u.id, newEarnings);
                bulkUpdates.push({ id: u.id, balance: u.balance + changes.userUpdates[uid].balanceChange, totalEarnings: newEarnings, ...(levelUp || {}) });
            }
        });
    ...
    // ... Giữ lại các hàm khác: handleUpdateUser, handleDeleteUser, handleSetPin, v.v.
