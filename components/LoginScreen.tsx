...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, onFinalize2faLogin, onRegister, onForgotPassword, referrerId, referrerName, isMaintenanceMode, maintenanceEndTime }) => {
    ...
    const handleGoogleRegister = async () => {
        ...
        setIsLoading(true);
        // Mô phỏng quá trình Google Login UI
        await new Promise(resolve => setTimeout(resolve, 1500));
        
        // Sinh ngẫu nhiên một user Google giả định
        const randomId = Math.floor(1000 + Math.random() * 9000);
        const mockName = `Google User ${randomId}`;
        const mockEmail = `user${randomId}@gmail.com`;
        const mockPassword = `GoogleAuth#${randomId}`; // Mật khẩu sinh tự động hợp lệ
        const mockPhone = `09${Math.floor(10000000 + Math.random() * 89999999)}`;
        
        // onRegister hiện tại cần password
        const result = await onRegister(mockName, mockEmail, mockPassword, referralCode, mockPhone);
        
        if (result.success) {
            // Bỏ qua bước xác minh email đối với đăng ký bằng Google
            // Thực hiện login luôn bằng tài khoản vừa tạo
            const loginResult = await onLogin(mockEmail, mockPassword);
