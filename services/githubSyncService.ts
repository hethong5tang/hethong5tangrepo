...
export const githubSyncService = {
  async request(endpoint: string, method: string, token: string, body?: any) {
  ...
  async pushAtomic(owner: string, repo: string, branch: string, token: string, files: GitHubFile[], logCallback: (msg: string) => void) {
    try {
      ...
      try {
        const refData = await this.request(`/repos/${owner}/${repo}/git/ref/heads/${branch}`, 'GET', token);
      ...
      if (latestCommitSha) {
        ...
        const commitData = await this.request(`/repos/${owner}/${repo}/git/commits/${latestCommitSha}`, 'GET', token);
      ...
      const newTreeData = await this.request(`/repos/${owner}/${repo}/git/trees`, 'POST', token, treeParams);
      ...
      const newCommitData = await this.request(`/repos/${owner}/${repo}/git/commits`, 'POST', token, commitParams);
